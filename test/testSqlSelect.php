...
class testSqlBuilder extends PHPUnit_Framework_TestCase
{
    ...
    public function testSqlSelectWithAlias()
    {
        ...
        $this->assertEquals('SELECT A.* FROM superA AS A LEFT JOIN hiperB AS B ON A.id = B.idSuperA WHERE A.name = (?) AND A.age = (?) GROUP BY A.id ORDER BY B.id', $queryOrder->sqlSelect());
